...
def createModelUNet(inputShape, dropoutRate, convLayer):
    ...
    #1st Stage
    conv1a = keras.layers.Conv2D(convLayer, (3, 3), activation='relu', padding = 'valid', name='conv1a')(inputLayer)
    drop1a = keras.layers.SpatialDropout2D(dropoutRate, name='drop1a')(conv1a)
    conv1b = keras.layers.Conv2D(convLayer, (3, 3), activation='relu', padding='valid', name='conv1b')(drop1a)
    ...
    #2nd Stage
    conv2a = keras.layers.Conv2D(convLayer*2, (3, 3), activation='relu', padding = 'valid', name='conv2a')(pool1)
    drop2a = keras.layers.SpatialDropout2D(dropoutRate, name='drop2a')(conv2a)
    conv2b = keras.layers.Conv2D(convLayer*2, (3, 3), activation='relu', padding = 'valid', name='conv2b')(drop2a)
    ...
    #3rd Stage
    conv3a = keras.layers.Conv2D(convLayer*4, (3, 3), activation='relu', padding = 'valid', name='conv3a')(pool2)
    drop3a = keras.layers.SpatialDropout2D(dropoutRate, name='drop3a')(conv3a)
    conv3b = keras.layers.Conv2D(convLayer*4, (3, 3), activation='relu', padding = 'valid', name='conv3b')(drop3a)
    ...
    #4th Stage
    conv4a = keras.layers.Conv2D(convLayer*8, (3, 3), activation='relu', padding = 'valid', name='conv4a')(pool3)
    drop4a = keras.layers.SpatialDropout2D(dropoutRate, name='drop4a')(conv4a)
    conv4b = keras.layers.Conv2D(convLayer*8, (3, 3), activation='relu', padding = 'valid', name='conv4b')(drop4a)
    ...
    #5th Stage
    conv5a = keras.layers.Conv2D(convLayer*16, (3, 3), activation='relu', padding = 'valid', name='conv5a')(pool4)
    drop5a = keras.layers.SpatialDropout2D(dropoutRate, name='drop5a')(conv5a)
    conv5b = keras.layers.Conv2D(convLayer*16, (3, 3), activation='relu', padding = 'valid', name='conv5b')(drop5a)
    drop5b = keras.layers.SpatialDropout2D(dropoutRate, name='drop5b')(conv5b)
    
    #6th Stage
    up6 = keras.layers.Conv2DTranspose(convLayer*8, (3, 3), activation='relu', padding = 'valid', name='conv6a')(drop5b)
    
    model = keras.models.Model(inputLayer, drop5b)
    model.summary()
    
    merge6 = keras.layers.concatenate([drop4b, up6], axis=3, name='merge6')
    conv6b = keras.layers.Conv2D(convLayer*8, (3, 3), activation='relu', padding = 'valid', name='conv6b')(merge6)
    drop6b = keras.layers.SpatialDropout2D(dropoutRate, name='drop6b')(conv6b)
    conv6c = keras.layers.Conv2D(convLayer*8, (3, 3), activation='relu', padding = 'valid', name='conv6c')(drop6b)
    drop6c = keras.layers.SpatialDropout2D(dropoutRate, name='drop6c')(conv6c)
    
    #7th Stage
    up7 = keras.layers.Conv2DTranspose(convLayer*4, (3, 3), activation='relu', padding = 'valid', name='conv7a')(drop6c)
    merge7 = keras.layers.concatenate([drop3b, up7], axis=3, name='merge7')
    conv7b = keras.layers.Conv2D(convLayer*4, (3, 3), activation='relu', padding = 'valid', name='conv7b')(merge7)
    drop7b = keras.layers.SpatialDropout2D(dropoutRate, name='drop7b')(conv7b)
    conv7c = keras.layers.Conv2D(convLayer*4, (3, 3), activation='relu', padding = 'valid', name='conv7c')(drop7b)
    drop7c = keras.layers.SpatialDropout2D(dropoutRate, name='drop7c')(conv7c)
    
    #8th Stage
    up8 = keras.layers.Conv2DTranspose(convLayerr*2, (3, 3), activation='relu', name='conv8a')(drop7c)
    merge8 = keras.layers.concatenate([drop2b, up8], axis=3, name='merge8')
    conv8b = keras.layers.Conv2D(convLayer*2, (3, 3), activation='relu', padding = 'valid', name='conv8b')(merge8)
    drop8b = keras.layers.SpatialDropout2D(dropoutRate, name='drop8b')(conv8b)
    conv8c = keras.layers.Conv2D(convLayer*2, (3, 3), activation='relu', padding = 'valid', name='conv8c')(drop8b)
    drop8c = keras.layers.SpatialDropout2D(dropoutRate, name='drop8c')(conv8c)
    
    #9th Stage
    up9 = keras.layers.Conv2DTranspose(convLayer, (3, 3), activation='relu', padding = 'valid', name='conv9a')(drop8c)
    merge9 = keras.layers.concatenate([drop1b, up9], axis=3, name='merge9')
    conv9b = keras.layers.Conv2D(convLayer, (3, 3), activation='relu', padding = 'valid', name='conv9b')(merge9)
    drop9b = keras.layers.SpatialDropout2D(dropoutRate, name='drop9b')(conv9b)
    conv9c = keras.layers.Conv2D(convLayer, (3, 3), activation='relu', padding = 'valid', name='conv9c')(drop9b)
    drop9c = keras.layers.SpatialDropout2D(dropoutRate, name='drop9c')(conv9c)
    
    #10th Stage
    conv10 = keras.layers.Conv2D(convLayer, (3, 3), activation='relu', padding = 'valid', name='conv10')(drop9c)
    
    model = keras.models.Model(inputs=inputLayer, output=conv10)
